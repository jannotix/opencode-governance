...
def main():
    with tempfile.TemporaryDirectory(prefix="opencode-v350-") as td:
        ...
        assert bug["debug_first_required"] is True and bug["tdd_required"] is True and bug["eval_required"] is False and bug["self_check_required"] is True
        docs = invoke("initialize-profile", "--project-dir", project, "--task-id", "DOC-1", "--work-class", "PATCH", "--task-kind", "DOCS", "--risks", "")
        assert docs["debug_first_required"] is False and docs["tdd_required"] is False and docs["eval_required"] is False
        ...
        assert ai["tdd_required"] is True and ai["eval_required"] is True and ai["required_reliability_mode"] == "PASS_K"
        ...
        assert blocked["status"] == "ARCHITECTURE_REVIEW_REQUIRED" and blocked["architecture_review_required"] is True
        ...
        passed = invoke("validate-debug", "--project-dir", project, "--task-id", "BUG-1", "--input-json", debug_good)
        ...
        assert not_ready["status"] == "NOT_READY_FOR_REVIEW" and not_ready["approval_authority"] is False
        ...
        assert ready["status"] == "READY_FOR_REVIEW" and ready["approval_authority"] is False
        ...
        added = invoke("add-learning", "--project-dir", project, "--input-json", candidate)
        assert added["promotion_status"] == "PENDING"
        duplicate = invoke("add-learning", "--project-dir", project, "--input-json", candidate, expect=3)
        assert duplicate["status"] == "DUPLICATE_LEARNING_CANDIDATE"
        ...
        denied = invoke("promote-learning", "--project-dir", project, "--input-json", unauthorized, expect=3)
        assert denied["status"] == "FINAL_REVIEWER_APPROVAL_REQUIRED"
        ...
        assert promotion["status"] == "PROMOTED" and promotion["memory_updated"] is False
        assert memory.read_text(encoding="utf-8") == "MEMORY_SENTINEL\n"

        validation = invoke("validate-task", "--project-dir", project, "--task-id", "BUG-1")
