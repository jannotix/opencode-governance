...
code=$?
set -e
[[ $code -eq 2 ]] || { echo "Expected exit 2 without next_action, got $code" >&2; exit 1; }
python3 -c 'import json,sys;assert json.loads(sys.argv[1])["error"]=="ACTIONABLE_CONTINUATION_REQUIRED"' "$output"
