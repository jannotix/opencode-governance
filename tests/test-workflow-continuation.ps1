...
$Action=[ordered]@{kind='execute';command='/ai-plan';arguments=@();expected_postcondition='READY_FOR_EXECUTION'}
try{
    New-Item -ItemType Directory -Force -Path $Temp|Out-Null
    $RunState=Join-Path $Temp 'RUN_STATE.json'
    @{top_level_command='ai-workflow';current_phase='AUDIT_PASS';next_required_phase='IDEA_INTAKE';terminal_reason=$null;next_action=$Action}|ConvertTo-Json -Depth 6|Set-Content -LiteralPath $RunState -Encoding utf8
    ...
    foreach($Case in @(
        ...
        @{top_level_command='ai-workflow';current_phase=$Case[0];next_required_phase=$Case[1];terminal_reason=$null;next_action=$Action}|ConvertTo-Json -Depth 6|Set-Content -LiteralPath $RunState -Encoding utf8
    ...
    $Output=& pwsh -NoProfile -File $Core -RunStatePath $RunState -ExpectedCommand ai-workflow 2>&1
    if($LASTEXITCODE-ne2-or(($Output-join"`n")|ConvertFrom-Json).error-ne'ACTIONABLE_CONTINUATION_REQUIRED'){throw 'Expected actionable continuation requirement'}
