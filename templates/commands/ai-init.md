...
Set `BASELINE_DRAFT`, create `.ai/baseline-audits/<AUDIT-ID>/`, and request independent `reviewer` and `reviewer-architecture` `BASELINE_AUDIT` reports against the same repository reference and draft baseline/index/documentation scope. Neither reviewer may see the sibling report. Then invoke `final-reviewer` in `BASELINE_AUDIT` mode.
...
On `BASELINE_PASS`, set `BASELINE_VALIDATED`, record the validated repository reference, baseline/index freshness and append evidence to `.ai/PROJECT_HISTORY.md`.

Initialize `PROJECT_HISTORY.md` as append-only. Never store secret values.

Do not rebuild/re-audit a valid baseline merely because `/ai-init` is run again. Use `/ai-audit` for explicit/material revalidation.
