...
4. Delegate exactly once to the selected public or hidden Executor route. Include `EXECUTION_MODE: ISOLATED_FAILOVER`, `EXECUTION_ROOT`, `EXECUTOR_ATTEMPT_ID`, selected route/model/variant/family, `WORK_CLASS`, `PACKET_SHA256`, `FROZEN_TARGET_SHA` and the same canonical execution packet. Require all writes and mutating commands to stay inside `EXECUTION_ROOT`.
