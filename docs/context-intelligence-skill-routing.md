...
OpenCode Governance provides a deterministic local-first context layer for bounded retrieval, skill capability selection, content-summary reuse and efficiency metrics. Path containment, required-section routing, terminal-state validation and workflow-continuation gates are part of the current 3.7.5 surface.
...
Current product version is **3.7.5**. Routing verification still accepts older installed manifests through **3.3.0+** (see `verify-routing`).
