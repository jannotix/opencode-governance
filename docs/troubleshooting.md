...
- Upgrade to Governance 3.7.5+, reinstall, run `verify` / `verify-routing`, then retry.
- If the workspace root is not the Git repository (for example a non-Git outer workspace with `Source_Code/` nested Git), install **3.7.5+** and pass explicit `-WorkspaceDir` / `-RepositoryDir` (or rely on unique nested-Git resolution). Managed roots include both `workspace/.ai` and `repository/.ai`.
