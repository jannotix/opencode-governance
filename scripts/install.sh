...
eligible=settings.get('eligible_failures')
...
for role in role_names:
    ...
    if role not in alias_roles and role!='architect' and fallbacks: raise SystemExit(f'{role} does not support fallback aliases.')
PY
fi
...
if [[ -n "$ROUTING_CONFIG" ]]; then
  ...
  if [[ -z "$backup_dir" || ! -d "$backup_dir" ]]; then
    echo 'Installer backup directory was not created; refusing to overwrite managed tools.' >&2
    exit 1
  fi
  for name in architect-attempt.ps1 architect-attempt.sh context-intelligence.ps1 context-intelligence.sh context-intelligence.py; do
    [[ ! -f "$tools/$name" ]] || cp -p "$tools/$name" "$backup_dir/$name"
  done
...
data['governance_version'] = '3.4.1'
data['architect_runner_version'] = '3.4.1'
data['context_intelligence_version'] = '3.4.1'
