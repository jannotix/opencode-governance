...
@(
  ...
  'commands\ai-plan.md','commands\ai-execute.md','commands\ai-review.md','commands\ai-workflow.md','commands\ai-status.md'
) | ForEach-Object {
    $Path = Join-Path $ConfigDir $_
    if (Test-Path $Path) { Remove-Item $Path -Force }
}
Write-Host 'Governance agents and commands removed. Existing provider authentication and backups were left untouched.'
