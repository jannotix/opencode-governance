...
if (Test-Path -LiteralPath $ManifestPath -PathType Leaf) {
    ...
    if ($Version -in @('3.3.2','3.3.3','3.3.4','3.4.0')) {
        ...
        $Remove=@($Expected[0],$Expected[1]);if($Version-eq'3.4.0'){$Remove+=$Expected[4..6]}
        ...
        $Manifest.PSObject.Properties.Remove('context_intelligence_version')
