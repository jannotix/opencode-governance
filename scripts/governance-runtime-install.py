...
def policy_blocks(tool_paths: dict[str, pathlib.Path]) -> dict[str, str]:
...
The repository test harness must exercise all twelve `/ai-*` command contracts with deterministic scenarios and must reject automatic push, merge, deployment, publication and production rollback. Simulation proves deterministic orchestration behavior, not live-model quality, and supplements rather than replaces repository-native verification.
""",
        "continuation": f"""
...
def install(source: pathlib.Path, config: pathlib.Path) -> None:
    ...
    backup_dir = backup(config, [*destination_tools, *agent_paths, *command_paths, resolved["manifest"]])
    ...
    blocks = policy_blocks(tools)
    for name in AGENT_NAMES:
        path = config / "agents" / f"{name}.md"
        text = path.read_text(encoding="utf-8")
        text = insert_section(text, "GOVERNANCE_AUTHORITY_V1", blocks["authority"], before_core=name in {"architect", "build", "plan"})
        text = insert_section(text, "GOVERNED_ENGINEERING_MEMORY_V2", blocks["memory"], before_core=name in {"architect", "build", "plan"})
        text = insert_section(text, "EVIDENCE_REUSE_LEDGER_V1", blocks["evidence"], before_core=name in {"architect", "build", "plan"})
        if name in {"reviewer", "reviewer-architecture", "final-reviewer"}:
            text = insert_section(text, "REVIEW_LENS_MATRIX_V1", blocks["lenses"])
    ...
    command_sections = {
        "ai-review": ("GOVERNANCE_AUTHORITY_ENTRY_V1", blocks["authority"] + "\n" + blocks["lenses"]),
        "ai-workflow": ("GOVERNANCE_RUNTIME_ENTRY_V1", blocks["authority"] + "\n" + blocks["continuation"] + "\n" + blocks["memory"] + "\n" + blocks["evidence"]),
        "ai-resume": ("GOVERNANCE_RUNTIME_ENTRY_V1", blocks["authority"] + "\n" + blocks["continuation"] + "\n" + blocks["memory"] + "\n" + blocks["evidence"]),
        "ai-release": ("GOVERNANCE_RELEASE_RECEIPT_GATE_V1", blocks["authority"] + "\nValidate the same content-bound receipt at the release candidate projection; release assessment never fabricates or silently renews approval."),
        "ai-status": ("GOVERNANCE_RUNTIME_STATUS_V1", "Report active candidate projection, receipt validity, actionable continuation, memory candidates/review-due state and evidence reuse/staleness without exposing source contents or secrets."),
        "ai-metrics": ("GOVERNANCE_RUNTIME_METRICS_V1", "Report receipt validations, projection mismatches, selected review lenses, memory admissions/rejections, evidence reuse hits/stale misses and simulation results when recorded. Do not fabricate unavailable token or cost data."),
    }
    for name, (heading, body) in command_sections.items():
        path = config / "commands" / f"{name}.md"
        path.write_text(insert_section(path.read_text(encoding="utf-8"), heading, body), encoding="utf-8")
    ...
    print(json.dumps({"status": "GOVERNANCE_RUNTIME_INSTALLED", "version": VERSION, "backup_dir": str(backup_dir)}, sort_keys=True))


def verify(config: pathlib.Path) -> None:
    resolved = paths(config)
    ...
    required_agent_markers = ("GOVERNANCE_AUTHORITY_V1", "GOVERNED_ENGINEERING_MEMORY_V2", "EVIDENCE_REUSE_LEDGER_V1")
    for name in AGENT_NAMES:
        text = (config / "agents" / f"{name}.md").read_text(encoding="utf-8")
        for marker in required_agent_markers:
            if text.count(f"## {marker}") != 1:
                fail("AGENT_RUNTIME_MARKER_INVALID", f"{name}:{marker}")
    for name in COMMAND_NAMES:
        text = (config / "commands" / f"{name}.md").read_text(encoding="utf-8")
        if "GOVERNANCE_" not in text:
            fail("COMMAND_RUNTIME_MARKER_MISSING", name)
    print(json.dumps({"status": "GOVERNANCE_RUNTIME_VERIFIED", "version": VERSION}, sort_keys=True))


def uninstall(config: pathlib.Path) -> None:
    resolved = paths(config)
    if not resolved["manifest"].is_file():
        print(json.dumps({"status": "GOVERNANCE_RUNTIME_NOT_INSTALLED"}, sort_keys=True))
        return
    manifest = json.loads(resolved["manifest"].read_text(encoding="utf-8-sig"))
    expected = [resolved["tools"] / name for name in TOOL_NAMES]
    if manifest.get("schema") != SCHEMA or manifest.get("managed_tools") != [str(path) for path in expected]:
        fail("UNSAFE_RUNTIME_MANIFEST")
    affected = [
        *expected,
    ...
    headings = (
        ...
        "GOVERNED_ENGINEERING_MEMORY_V2",
        "EVIDENCE_REUSE_LEDGER_V1",
    ...
    for directory, names in (("agents", AGENT_NAMES), ("commands", COMMAND_NAMES)):
        for name in names:
            path = config / directory / f"{name}.md"
            text = path.read_text(encoding="utf-8")
    ...
    print(json.dumps({"status": "GOVERNANCE_RUNTIME_REMOVED", "backup_dir": str(backup_dir)}, sort_keys=True))
