...
case "$version" in 3.3.2|3.3.3|3.3.4|3.4.0|3.4.1|3.4.2);;*) echo "Unsupported routing manifest governance_version: $version" >&2; exit 1;; esac
...
if version in {'3.3.3','3.3.4','3.4.0','3.4.1','3.4.2','3.4.2','3.4.2'}: policy += ['POWERSHELL_7_REQUIRED','pwsh -NoProfile -File']
if version in {'3.3.4','3.4.0','3.4.1','3.4.2','3.4.2'}: policy += ['PROJECT_STATE_FINGERPRINT_V1','NON_GIT_PROJECT_SUPPORTED','PROJECT_STATE_CHANGED']
if version in {'3.4.0','3.4.1','3.4.2'}: policy += ['CONTEXT_INTELLIGENCE_V1','CONTEXT_BUDGET.json','SKILL_CAPABILITY_MANIFEST_V1','CONTEXT_SUFFICIENT','BLOCKED_CONTEXT_GAP',str(expected[4]),str(expected[5]),str(expected[6])]
if version in {'3.4.1','3.4.2'}: policy += ['Governance state paths may not traverse symbolic links or reparse points.']
...
if version in {'3.3.3','3.3.4','3.4.0','3.4.1','3.4.2','3.4.2','3.4.2'}: gate += ['pwsh -NoProfile -File']
if version in {'3.3.4','3.4.0','3.4.1','3.4.2','3.4.2'}: gate += ['PROJECT_STATE_CHANGED']
...
if version in {'3.3.4','3.4.0','3.4.1','3.4.2','3.4.2'}:
    ...
    if version in {'3.4.1','3.4.2'} and 'default cooldown must be an integer between 60 and 86400 seconds.' not in ps_runner: raise SystemExit('PowerShell Architect runner missing cooldown validation.')
if version in {'3.4.0','3.4.1','3.4.2'}:
