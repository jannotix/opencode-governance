...
function Prepare-Attempt([object]$Routing) {
    ...
    if (-not (Test-Path -LiteralPath $launchHelper -PathType Leaf)) {
        throw 'EFFECT_PLUGIN_NOT_ACTIVE: governed-role-launch.py missing'
    }
    $launchPath = Join-Path (Split-Path -Parent $Paths.manifest) 'governed-role-launch-executor.json'
    $handshakePath = Join-Path (Split-Path -Parent $Paths.manifest) 'handshake-executor.json'
    ...
    $requirePlugin = Test-Path -LiteralPath $effectPolicy -PathType Leaf
    try {
        $rm = Get-Content -LiteralPath (Join-Path $ConfigDir 'opencode-governance-routing.json') -Raw | ConvertFrom-Json
        if ($rm.effect_plugin_sha256 -and $rm.effect_policy_sha256) { $requirePlugin = $true }
    } catch {}
    $wargs = @(
    ...
    if ($requirePlugin) { $wargs += @('--require-plugin') }
    $written = & python @wargs 2>&1
    if ($LASTEXITCODE -ne 0) {
        throw "GOVERNED_ROLE_LAUNCH_REQUIRED: $written"
    }
    $launchSha = ''
    try {
        $launchObj = ($written | Select-Object -Last 1 | ConvertFrom-Json)
        if ($launchObj.sha256) { $launchSha = [string]$launchObj.sha256 }
    } catch {}
    if (-not $launchSha -and (Test-Path -LiteralPath $launchPath)) {
        $launchSha = (Get-FileHash -LiteralPath $launchPath -Algorithm SHA256).Hash.ToLowerInvariant()
    }
    $Data | Add-Member NoteProperty governed_role_launch_file $launchPath -Force
    $Data | Add-Member NoteProperty governed_role_launch_sha256 $launchSha -Force
    $Data | Add-Member NoteProperty open_code_governance_role 'executor' -Force
    $Data | Add-Member NoteProperty required_role_runner 'governed-role-attempt.py' -Force
    Write-Json $Data $Paths.manifest
    [pscustomobject]@{
        execution_root = $Worktree
        ...
        governed_role_launch_sha256 = $launchSha
        required_role_runner = 'governed-role-attempt.py'
        OPENCODE_GOVERNANCE_LAUNCH_FILE = $launchPath
        OPENCODE_GOVERNANCE_LAUNCH_SHA256 = $launchSha
        OPENCODE_GOVERNANCE_HANDSHAKE_PATH = $handshakePath
        OPENCODE_GOVERNANCE_EFFECT_ENFORCEMENT_ACTIVE = '1'
        OPENCODE_GOVERNANCE_ROLE = 'executor'
        OPENCODE_GOVERNANCE_EXPECTED_AGENT = [string]$RouteAgent
