...
foreach ($Marker in $EvidenceMarkers) { if ($AgentText['architect'] -notmatch [regex]::Escape($Marker)) { throw "Architect is missing Evidence-Driven Verification marker $Marker." } }
foreach ($Name in @('build','plan','executor','reviewer','reviewer-architecture','final-reviewer')) {
    foreach ($Marker in @('VERIFICATION_PROFILE','TASK_RISK_PROFILE')) { if ($AgentText[$Name] -notmatch [regex]::Escape($Marker)) { throw "$Name is missing Evidence-Driven Verification marker $Marker." } }
}
...
foreach ($Marker in $OperationalMarkers) { if ($AgentText['architect'] -notmatch [regex]::Escape($Marker)) { throw "Architect is missing v2.0 Operational Assurance marker $Marker." } }
foreach ($Name in @('build','plan','executor','reviewer','reviewer-architecture','final-reviewer')) {
    foreach ($Marker in $OperationalMarkers) { if ($AgentText[$Name] -notmatch [regex]::Escape($Marker)) { throw "$Name is missing v2.0 Operational Assurance marker $Marker." } }
}
foreach ($Name in @('plan','executor','reviewer','reviewer-architecture','final-reviewer')) {
    foreach ($Risk in @('USER_FLOW','VISUAL_BEHAVIOR','EXTERNAL_TOOLING','RECOVERY','EXPERIMENTATION')) { if ($AgentText[$Name] -notmatch [regex]::Escape($Risk)) { throw "$Name is missing v2.0 risk dimension $Risk." } }
}
if ($AgentText['executor'] -notmatch 'external_directory:\s+deny') { throw 'Executor must preserve external_directory deny; Operational Assurance must not broaden permissions.' }
...
foreach ($Marker in $OperationalMarkers) { if ($PlanCommand -notmatch [regex]::Escape($Marker) -or $WorkflowCommand -notmatch [regex]::Escape($Marker)) { throw "Operational Assurance planning/workflow is missing $Marker." } }
foreach ($Marker in @('OPERATIONAL_PLANNING','OPERATIONAL_VALIDATION')) { if ($WorkflowCommand -notmatch [regex]::Escape($Marker)) { throw "/ai-workflow is missing v2.0 lifecycle marker $Marker." } }
foreach ($Name in @('ai-execute','ai-review','ai-status','ai-resume','ai-release')) {
    ...
    if ($Text -notmatch 'OPERATIONAL_ASSURANCE') { throw "/$Name is missing v2.0 Operational Assurance handling." }
}
foreach ($Marker in $OperationalMarkers[1..7]) {
    foreach ($Name in @('ai-execute','ai-review','ai-status','ai-release')) { if ($CommandText[$Name] -notmatch [regex]::Escape($Marker)) { throw "/$Name is missing operational gate $Marker." } }
}
if ($InitCommand -notmatch 'Operational Assurance' -or $InitCommand -notmatch 'read-only') { throw '/ai-init is missing read-only Operational Assurance capability discovery.' }
if ($AuditCommand -notmatch 'Operational Assurance' -or $AuditCommand -notmatch 'read-only') { throw '/ai-audit is missing read-only Operational Assurance capability discovery.' }
...
foreach ($Marker in @('RUN_STATE.json','STEERING.md','GOVERNANCE_RESULT','ENVIRONMENT_FINGERPRINT','STALE','OPERATIONAL_ASSURANCE')) { if ($ResumeCommand -notmatch [regex]::Escape($Marker)) { throw "/ai-resume is missing $Marker." } }
