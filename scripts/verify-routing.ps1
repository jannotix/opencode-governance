...
$Supported=@('3.3.2','3.3.3','3.3.4','3.4.0','3.4.1','3.4.2','3.4.3','3.4.4')
...
if($Version-in@('3.4.0','3.4.1','3.4.2','3.4.3','3.4.4')){
    ...
    if($Version-eq'3.4.4'){
        if([string]$Manifest.workflow_continuation_version-ne'3.4.4'){throw 'workflow_continuation_version must be 3.4.4.'}
...
if($Version-in@('3.4.1','3.4.2','3.4.3','3.4.4')){$PolicyMarkers+='Governance state paths may not traverse symbolic links or reparse points.'}
...
if($Version-eq'3.4.4'){$GateMarkers+=@('WINDOWS_COMMAND:','UNIX_COMMAND:','-ProjectDir','--project-dir','<ORIGINAL_ARGUMENTS>')}
...
if($Version-in@('3.3.4','3.4.0','3.4.1','3.4.2','3.4.3','3.4.4')){
    ...
    if($Version-in@('3.4.1','3.4.2','3.4.3','3.4.4')-and$PowerShellRunner-notlike'*default cooldown must be an integer between 60 and 86400 seconds.*'){throw 'PowerShell Architect runner missing cooldown validation.'}
}
if($Version-in@('3.4.0','3.4.1','3.4.2','3.4.3','3.4.4')){
    ...
    if($Version-in@('3.4.1','3.4.2','3.4.3','3.4.4')){foreach($Value in @('GOVERNANCE_STATE_LINK_FORBIDDEN','REQUIRED_SECTION_UNAVAILABLE','TERMINAL_STATE_REQUIRED')){if($PsContext-notlike"*$Value*"-or$PyContext-notlike"*$Value*"){throw "Context hardening marker missing: $Value"}}}
}
if($Version-eq'3.4.4'){
