...
$Supported=@('3.3.2','3.3.3','3.3.4','3.4.0','3.4.1')
...
if($Version-in@('3.4.0','3.4.1')){
    $ExpectedRunner=if($Version-eq'3.4.0'){'3.3.4'}else{'3.4.1'}
    $ExpectedContext=if($Version-eq'3.4.0'){'3.4.0'}else{'3.4.1'}
...
if($Version-in@('3.3.3','3.3.4','3.4.0','3.4.1')){$PolicyMarkers+=@('POWERSHELL_7_REQUIRED','pwsh -NoProfile -File')}
if($Version-in@('3.3.4','3.4.0','3.4.1')){$PolicyMarkers+=@('PROJECT_STATE_FINGERPRINT_V1','NON_GIT_PROJECT_SUPPORTED','PROJECT_STATE_CHANGED')}
if($Version-in@('3.4.0','3.4.1')){$PolicyMarkers+=@('CONTEXT_INTELLIGENCE_V1','CONTEXT_BUDGET.json','SKILL_CAPABILITY_MANIFEST_V1','CONTEXT_SUFFICIENT','BLOCKED_CONTEXT_GAP',$ExpectedTools[4],$ExpectedTools[5],$ExpectedTools[6])}
if($Version-eq'3.4.1'){$PolicyMarkers+='Governance state paths may not traverse symbolic links or reparse points.'}
...
if($Version-in@('3.3.3','3.3.4','3.4.0','3.4.1')){$GateMarkers+='pwsh -NoProfile -File'}
if($Version-in@('3.3.4','3.4.0','3.4.1')){$GateMarkers+='PROJECT_STATE_CHANGED'}
...
if($Version-in@('3.4.0','3.4.1')){foreach($Command in @('ai-workflow','ai-resume','ai-metrics')){$Text=Get-Content -LiteralPath (Join-Path $ConfigDir "commands/$Command.md") -Raw;foreach($Value in @('CONTEXT_INTELLIGENCE_ENTRY','BLOCKED_CONTEXT_GAP',$ExpectedTools[4],$ExpectedTools[5])){if($Text-notlike"*$Value*"){throw "$Command missing Context Intelligence marker: $Value"}}}}

if($Version-in@('3.3.4','3.4.0','3.4.1')){
    ...
    if($Version-eq'3.4.1'-and$PowerShellRunner-notlike'*default cooldown must be between 60 and 86400 seconds.*'){throw 'PowerShell Architect runner missing cooldown validation.'}
}
if($Version-in@('3.4.0','3.4.1')){
    ...
    if($Version-eq'3.4.1'){foreach($Value in @('GOVERNANCE_STATE_LINK_FORBIDDEN','REQUIRED_SECTION_UNAVAILABLE','TERMINAL_STATE_REQUIRED')){if($PsContext-notlike"*$Value*"-or$PyContext-notlike"*$Value*"){throw "Context hardening marker missing: $Value"}}}
}
